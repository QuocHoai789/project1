		...
		<div id="doc">
			<ul>
				...
				while($d=mysqli_fetch_array($kq3)) { 
					?>
					<li><a href="http://localhost:81/do_an_web/loaisanpham.php?MaLoai=<?php echo $d['MaLoai'] ; ?>"><?php echo $d['TenLoai']; ?></a> </li>
				<?php } ?>	
			</ul>
		</div>
	</div>
	...
	// }
		...
		while($d=mysqli_fetch_array($kq4)) { 
			...
			<div class="sp2a"><div class="chua" style="position: relative;"><img class="imge" src="<?php echo $d['HinhAnh']; ?>">
				...
				<p class="tensp"><a href="http://localhost:81/do_an_web/chitietsanpham.php?MaSp=<?php echo $d['MaSp']; ?>"><?php echo $d['TenSp']; ?></a></p>
		...
		<br/>
		<p  id="pt" >
			<a href="http://localhost:81/do_an_web/sanpham.php?gr=<?php echo $gr-1;  if(isset($sapxep)) echo '&sapxep='.$sapxep;  if(isset($_GET['lsp'])) echo '&lsp='.$_GET['lsp']; ?>">
				<?php if($gr>1){echo '&laquo';} ?>

			</a>
			<?php for($i=$dau;$i<=$cuoi;$i++){?>
				<a href="http://localhost:81/do_an_web/sanpham.php?page=<?php echo $i;  if(isset($sapxep) ) echo '&sapxep=' .$sapxep;  if(isset($_GET['lsp'])) echo '&lsp='.$_GET['lsp']; ?>">
